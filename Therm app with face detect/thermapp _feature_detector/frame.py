...
R1, B, F, O, R2 = 17711.559, 1447.2, 0.57999998, -4096, 0.025931966

def raw_to_celsius(counts: float) -> float:
    c = max(counts, 1e-3)           
    T_k = B / np.log(R1/c + 1.0)
    return T_k - 273.15
...
class DisplayThread:
    ...
    def __init__(self, resize_factor=2):
        ...
        self.last_raw = None       # last 2D numpy array of raw counts
    ...
    def _mouse_callback(self, event, x, y, flags, param):
        if event == cv2.EVENT_MOUSEMOVE and self.last_raw is not None:
            h, w = self.last_raw.shape
            fx, fy = x // self.resize_factor, y // self.resize_factor
            if 0 <= fx < w and 0 <= fy < h:
                count = float(self.last_raw[fy, fx])
                temp_c = raw_to_celsius(count)
    ...
    def display_frame(self, raw_frame: np.ndarray):
        # 1) compute raw stats
        min_v = int(raw_frame.min())
        max_v = int(raw_frame.max())

        # 2) reshape to 2D
        h, w = 288, 384
        img2d = raw_frame.reshape((h, w)).astype(np.uint8)
        self.last_raw = img2d  # store for mouse callback

        # 3) prepare raw-BGR
        raw_bgr = cv2.cvtColor(img2d, cv2.COLOR_GRAY2BGR)
